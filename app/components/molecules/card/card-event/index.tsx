...
const CardEvent: React.FC<CardEventProps> = ({ children, title, date, startTime, location, description, submissionDate, status, isPast, eventImg, ...rest }) => {
    ...
    const fallbackCard = (
            <Card {...rest}
            styles={{ body: { padding: 16 } }}
        style={{ ...incomingStyle, border: '1.5px dashed var(--accent-gray-light-2)', background: 'var(--background-2)' }}>
            <div className='flex flex-col gap-1'>
                <Image
                    width={200}
                    alt="basic"
                    src={imagePath ?? eventImg ?? "https://zos.alipayobjects.com/rmsportal/jkjgkEfvpUPVyRjUImniVslZfWPnJuuZ.png"}
                />
                <Title level={4} ellipsis={{ rows: 2 }} className={styles.title}>{title}</Title>
                <Paragraph ellipsis={{ rows: 2 }}>{description}</Paragraph>
                <div className='flex flex-wrap'>
                    <Tag className="eventDetailTag" style={{background: isPast ? '#FFCCC7' : 'var(--background)'}} icon={<CalendarOutlined />}>{date}</Tag>
                    <Tag className="eventDetailTag" icon={<ClockCircleOutlined />}>{formatTime(startTime)}</Tag>
                    <Tag className="eventDetailTag" icon={<PushpinOutlined />}>{location}</Tag>
                </div>
            </div>
